...
select id_unidad
	...
	END try BEGIN catch
	SELECT
			'ERROR DE OPERACION: ' + Error_message();
	...
	IF @action = 'get_empresa' begin
		SELECT
			...
			SELECT
				...
				nombre
			FROM
				unidades_negocios AS un
			WHERE
				un.id_un_superior = 'raiz'
			FOR json path
		) data
	END
	...
	IF @action = 'login' BEGIN
		BEGIN TRY DECLARE @usuario TABLE (
	...
	END TRY BEGIN CATCH
	SELECT
			CAST(ERROR_NUMBER() AS NVARCHAR(50)) + ' ' + CAST(ERROR_LINE() AS NVARCHAR(50)) + ' ' + ERROR_MESSAGE() AS ErrorMessage
	END CATCH
	END
	...
	IF @action = 'upadte-user' BEGIN
		BEGIN TRY DECLARE @user1 TABLE (pass NVARCHAR(max),
			cedula varchar(50))
	insert into
		@user1
		select
			*
		from
			openjson(@json) WITH ([pass] NVARCHAR(max), [cedula] varchar(50));

	UPDATE
		USUARIOS
	SET
		CLAVE_SECUNDARIA = n.pass
	from
			@user1 as n
	WHERE
		USUARIO = n.cedula;
		...
		select
			...
			(
				...
				from
					ITM_DOMINIOS itD
				where
									ID_GRUPO_DOMINIO = 'BASES COSTO'
				for json path
							)
						) todos
			from
				ESPECIFICACIONES_PRODUCCION ep,
				@x as x
			where
						ep.ID_UN = x.id_unidad
	...
	IF @action = 'update_dominios' BEGIN
	...
	WHERE
		...
		select
			...
			select
				a.*
			from
				ESPECIFICACIONES_PRODUCCION as a,
				@BCOSTO as c
			where
				a.ID_UN = c.id_unidad
	...
	IF @action = 'getSecciones' BEGIN
		...
		SELECT
			CAST(@ID_UN AS NVARCHAR(150)) AS ID_UN,
			ID_UN AS ID_SECCION,
	...
	INSERT INTO
		...
		VALUES
			(
				...
				'NO EXISTE',
				0,
				'No hay definidas UN tipo Planta',
				0,
				'',
				0,
				'ACTIVO',
	...
	FROM
			#TSECC AS T
			INNER JOIN @TRAIZ AS R ON T.ID_SECCION = R.ID_UN_ITEM
		-- Incluye el resto de secciones
		IF @productos <> ''
	SET
		@SQLS = 'SELECT *,'''' FROM SECCIONES WHERE ' + @productos
		ELSE
	SET
	...
	IF @action = 'EditSeccion' BEGIN
		BEGIN TRY DECLARE @Seccion01 TABLE (
	...
	IF @action = 'EliminarSeccion' BEGIN
		BEGIN TRY
	select
			*
		from
	...
	IF @action = 'NuevaSeccion' BEGIN
		BEGIN TRY DECLARE @Seccion04 TABLE (
			[DESCRIPCION] [nvarchar](250) NULL,
			[ID_UN] [nvarchar](50) NOT NULL,
			[ANTERIOR] [int] NULL,
			[ASIGNABLE] [bit] NULL,
	...
	IF @action = 'getEmpleados' BEGIN
		...
		@seccion = (
			...
			seccion
		from
			openjson(@json) WITH ([seccion] nvarchar(30))
			)
		);
	...
	IF @action = 'getAllEmpleados' BEGIN
		select
			(
				SELECT
				S.ID_UN,
